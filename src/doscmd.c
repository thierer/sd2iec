...
/* handler e.g. in response to a M-E call. Will be reset to NULL after the */
/* next M-R command. On AVR the magic value must reside in RAM, not flash! */
magic_value_t *custom_magic = NULL;
...
static void handle_memread(void) {
  ...
  use_internal:
    if (custom_magic != NULL && custom_magic->address == address) {
      ...
      if (check == 0) {
        /* use 0 as defaults, so these need no table entries */
        error_buffer[0] = 0;
        error_buffer[1] = 0;
      }
    }
